...
pub fn parse_obj_into_computed_shape<P>(file_name: P, scale_factor: f32) -> Result<Vec<Collider>>
where
    P: AsRef<Path> + fmt::Debug,
{
    ...
    let colliders = model
        .iter()
        .map(|mesh| -> (Vec<Vect>, Vec<u32>) { grab_computed_shape_data(&mesh.mesh, scale_factor) })
        .map(|g| -> Collider {
...
fn grab_computed_shape_data(mesh: &tobj::Mesh, scale_factor: f32) -> (Vec<Vect>, Vec<u32>) {
...
pub fn parse_obj_into_trimeshes<P>(file_name: P, scale_factor: f32) -> Result<Vec<Collider>>
where
    P: AsRef<Path> + fmt::Debug,
{
    let (model, _) = load_obj(file_name, &tobj::GPU_LOAD_OPTIONS)?;
